...
.module('ngSchemaFormFile', [
	...
	.directive('ngSchemaFile', ['Upload', '$timeout', '$q','$rootScope', function (Upload, $timeout, $q,$rootScope) {
		return {
			restrict: 'A',
			scope:    true,
			require:  'ngModel',
					...
					if (file && !file.$error && scope.url) {
						...
						file.upload.then(function (response) {
							$timeout(function () {
								...
								if(!scope.uploadedFiles.includes(response.data.path)){
									scope.uploadedFiles.push(response.data.path);
								}
...
angular.module("schemaForm").run(["$templateCache", function($templateCache) 
	{$templateCache.put("directives/decorators/bootstrap/nwp-file/nwp-file.html",
			"<ng-form class=\"file-upload mb-lg\" " +
			"ng-schema-file ng-model=\"$$value$$\"" +
			" name=\"uploadForm\">\n" +
			"    <label ng-show=\"form.title && form.notitle !== true\" class=\"control-label\" for=\"fileInputButton\" ng-class=\"{\'sr-only\': !showTitle(), \'text-danger\': uploadForm.$error.required && !uploadForm.$pristine}\">\n        " +
			"{{ form.title }}<i ng-show=\"form.required\">&nbsp;*</i>\n    </label>\n    " +
			"<div ng-show=\"picFile\">\n        " +
			"<div ng-include=\"\'uploadProcess.html\'\" class=\"mb\">" +
			"</div>\n    " +
			"</div>\n    " +
			"<ul ng-show=\"picFiles && picFiles.length\" class=\"list-group\">\n        " +
			"<li class=\"list-group-item\" ng-repeat=\"picFile in picFiles\">\n            " +
			"<div ng-include=\"\'uploadProcess.html\'\">" +
			"</div>\n        " +
			"</li>\n    " +
			"</ul>\n    " +
			...
			"<small class=\"text-muted\" ng-show=\"form.description\" ng-bind-html=\"form.description\">" +
			"</small>\n        " +
			"<div ng-if=\"isSinglefileUpload\" ng-include=\"\'singleFileUpload.html\'\">" +
			"</div>\n        " +
			"<div ng-if=\"!isSinglefileUpload\" ng-include=\"\'multiFileUpload.html\'\">" +
			"</div>\n        " +
			"<div class=\"help-block mb0\" ng-show=\"uploadForm.$error.required && !uploadForm.$pristine\">{{ \'modules.attribute.fields.required.caption\' | translate }}</div>\n        " +
			"<div class=\"help-block mb0\" ng-show=\"(hasError() && errorMessage(schemaError()))\" ng-bind-html=\"(hasError() && errorMessage(schemaError()))\">" +
			"</div>\n    " +
			...
			"<div class=\"text-danger errorMsg\" ng-message=\"maxSize\">{{ form[picFile.$error].validationMessage | translate }} " +
			"<strong>{{picFile.$errorParam}}</strong>. " +
			"({{ form[picFile.$error].validationMessage2 | translate }} " +
			"<strong>{{picFile.size / 1000000|number:1}}MB</strong>)" +
			"</div>\n        " +
